...
authRouter.get("/callback", async (c) => {
  try {
    ...
    if (error) {
      return c.redirect(`${new URL(c.req.url).origin}?error=${error}`);
    }

    if (!code) {
      return c.redirect(`${new URL(c.req.url).origin}?error=no_code`);
    }
    ...
    if (!accessToken) {
      return c.redirect(`${new URL(c.req.url).origin}?error=no_token`);
    }
    ...
    if (existingUsers.length === 0) {
      ...
      if (user.isBanned) {
        return c.redirect(`${new URL(c.req.url).origin}?error=banned`);
      }
    ...
    return c.redirect(
      `${new URL(c.req.url).origin}/callback?token=${token}`,
    );
  } catch (error) {
    console.error("Auth error:", error);
    return c.redirect(`${new URL(c.req.url).origin}?error=auth_failed`);
  }
});
